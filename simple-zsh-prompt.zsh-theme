...
ZSH_THEME_GIT_PROMPT_NEED_TO_PUSH="%{$fg_bold[yellow]%}⤒%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_NEED_TO_ADD="%{$fg_bold[yellow]%}±%{$reset_color%}"
...
function git_status() {
  ...
  if [[ $branch != "" ]]; then
    ...
    if [[ $needToAdd -ne 0 ]]; then
      ...
      prompt="${prompt} ${ZSH_THEME_GIT_PROMPT_NEED_TO_PUSH} "
    else
